...
public class OkHttpRequest {
    ...
    public static void doExecute(final Request request, HttpCallback callback) {
        if (NetworkUtils.isNetworkAvailable()) {
            ...
            OkHttpUtils.sendNetErrorResultCallback(callback);
        }
            ...
            getOkHttpClient().newCall(request).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) {
                    ...
                    try {
                        ...
                        OkHttpUtils.sendSuccessResultCallback(file.getAbsolutePath(), callback);
                    } catch (IOException e) {
                        e.printStackTrace();
                        OkHttpUtils.sendFailResultCallback(-1, e.getMessage(), callback);
                    } finally {
    ...
    public static void doNewWebSocket(final Request request) {
        if(NetworkUtils.isNetworkAvailable()){
            getOkHttpClient().newWebSocket(request, new okhttp3.WebSocketListener() {
                @Override
